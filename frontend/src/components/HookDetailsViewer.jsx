...
const HookDetailsViewer = ({ hook }) => {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
      <h2 className="text-2xl font-bold mb-6 text-gray-900 dark:text-white">Webhook Details</h2>
      ...
      <div className="mb-6 border-b border-gray-200 dark:border-gray-700 pb-4">
        ...
        <dl className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
          ...
          <dd className="text-gray-900 dark:text-gray-200">{new Date(hook.created_at).toLocaleString()}</dd>
        </dl>
      </div>

      <div className="mb-6">
        <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">Headers</h3>
        <pre className="bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-gray-200 p-4 rounded-md text-sm whitespace-pre-wrap break-words">
      ...
      {hook.original_body && (
        <div className="mb-6">
          ...
          <pre className="bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-gray-200 p-4 rounded-md text-sm">
            {renderJson(hook.original_body)}
          </pre>
        </div>
      )}
      ...
      <div>
        <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">
          {hook.original_body ? 'Transformed Body' : 'Body'}
        </h3>
        <pre className="bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-gray-200 p-4 rounded-md text-sm">
          {renderJson(hook.body)}
        </pre>
      </div>
    </div>
  );
